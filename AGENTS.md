...
- **日志内容白名单**：绝不记录报告原文、证据文本、OCR 文本、姓名、原始过敏或医嘱文本、
  健康数据与凭证。LLM-A、OCR 以及任何可能包含体检、用户或健康信息的模型链路，禁止记录
  完整请求响应；仅处理公开菜品数据且不含用户与健康信息的 LLM-B 链路，允许记录完整请求响应。
  任务与用户标识可用于关联，但不得与上述隐私内容出现在同一条日志事件中，也不得进 URL
  查询串或分享链接。
