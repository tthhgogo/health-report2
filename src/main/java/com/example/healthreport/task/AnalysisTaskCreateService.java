...
/**
 * 分析任务创建服务。
 * <p>事务外预检由调用方先执行；本服务在事务内插入任务并重新锁行校验、绑定文件。</p>
 */
@Service
public class AnalysisTaskCreateService {
    ...
    @Autowired
    public AnalysisTaskCreateService(CtHealthReportTaskService taskService,
     ...
     */
    public void precheck(List<String> fileIdList, String userId) {
        fileBindingService.precheckFiles(fileIdList, userId);
    }

    /**
     * 在同一个数据库事务中创建任务并绑定全部文件。
     * <p>任何绑定异常都会以 RuntimeException 传播，触发任务插入和已完成绑定整体回滚。</p>
     */
    @Transactional(rollbackFor = Exception.class)
    public String createInTransaction(List<String> fileIdList, String userId) {
        ...
        fileBindingService.bindFiles(fileIdList, taskId, userId);
        return taskId;
    }
}
