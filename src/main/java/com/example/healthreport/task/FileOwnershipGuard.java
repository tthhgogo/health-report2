...
/**
 * 文件归属校验入口。
 * <p>查询只按 file_id 取行，随后在 Java 中对 user_id 做区分大小写的精确比较。</p>
 * <p>文件状态、有效期与可绑定性由文件绑定服务统一判断，本类不消费这些业务条件。</p>
 */
@Component
public class FileOwnershipGuard {
     ...
     */
    public CtHealthReportFileEntity assertOwned(String fileId, String currentUserId) {
        idCanonicalizer.canonicalize(fileId);
        CtHealthReportFileEntity fileEntity = fileService.findByFileId(fileId);
        if (fileEntity == null
                || !fileId.equals(fileEntity.getFileId())
                || currentUserId == null
                || !currentUserId.equals(fileEntity.getUserId())) {
    ...
    public List<CtHealthReportFileEntity> assertOwned(List<String> fileIdList, String currentUserId) {
        ...
        for (String fileId : fileIdList) {
            resultList.add(assertOwned(fileId, currentUserId));
        }
        return resultList;
    }
     ...
     * <p>本方法不再访问数据库，供事务外预检与事务内锁行校验各调用一次。</p>
     */
    public List<FileBindingRecord> assertOwnedRecords(
            List<String> fileIdList,
            List<FileBindingRecord> recordList,
            String currentUserId) {
        if (fileIdList == null || recordList == null || currentUserId == null) {
            throw new OwnershipException();
        }
        Map<String, FileBindingRecord> recordMap =
                new HashMap<String, FileBindingRecord>(recordList.size());
        for (FileBindingRecord record : recordList) {
            if (record != null && record.getFileId() != null) {
                recordMap.put(record.getFileId(), record);
            }
        }
        List<FileBindingRecord> orderedRecordList =
                new ArrayList<FileBindingRecord>(fileIdList.size());
        for (String fileId : fileIdList) {
            idCanonicalizer.canonicalize(fileId);
            FileBindingRecord record = recordMap.get(fileId);
            if (record == null || !fileId.equals(record.getFileId())
