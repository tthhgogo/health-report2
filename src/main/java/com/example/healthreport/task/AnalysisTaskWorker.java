 ...
 * 分析工作线程：解析 → 抽取 → 组装 → 写结果。
 * <p>
 * 三个阶段各自有编排类，本类只负责阶段顺序、状态机推进与失败收敛， <b>不实现任何业务规则</b>。
 * </p>
 */
@Slf4j
@Component
public class AnalysisTaskWorker {
	...
	public AnalysisTaskWorker(TaskStateService taskStateService, CtHealthReportTaskService taskService,
			TaskResourceCleanupService resourceCleanupService, TaskParseService taskParseService,
			ParseOrchestrator parseOrchestrator, ExtractionStageService extractionStageService,
	...
	public void run(String taskId) {
		...
		try {
			...
			log.info("任务解析阶段处理完成，taskId={}，输入文件数={}，处理页数={}，总页数={}，耗时={}ms", taskId, parsedFileList.size(),
					parsePlan.getProcessedPages(), parsePlan.getTotalPages(),
					System.currentTimeMillis() - stageStartMillis);
			...
			ValidatedExtractionOutput extractionOutput = extractionStageService.extract(parsePlan, accumulator);
			...
			AnalysisModules modules = analysisAssembleService.assemble(extractionOutput,
					parsePlan.getReadableFileList().size(), companyId, bizDate, accumulator);
			...
			AnalysisResult result = AnalysisResult.create(accumulator, parsePlan.getProcessedPages(),
					parsePlan.getTotalPages(), modules);
			if (taskStateService.markSucceeded(taskId, result)) {
				// 端到端耗时只有这里能算出来：三个阶段各自的耗时加起来不等于它，
				// 中间还有状态迁移、心跳与线程池排队。容量参数 W 要按这个数校准（§4.2）。
				log.info("任务分析全部完成，taskId={}，是否部分结果={}，处理页数={}，总页数={}，端到端耗时={}ms", taskId, accumulator.partial(),
						parsePlan.getProcessedPages(), parsePlan.getTotalPages(),
