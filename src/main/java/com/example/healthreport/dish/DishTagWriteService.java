...
public class DishTagWriteService {
    ...
    /**
     * B2 与 B3 唯一执行点：REJECT 的 evidence_type 必填；INGREDIENT 证据必须非空且属于菜品食材。
     * DDL 无兜底。
     */
     ...
     */
    public CtDishTagEntity toEntity(DishTagOutput.Hit hit, Dish dish, String tagHash,
                                    String enumKey, String modelVersion, String promptVersion,
                                    String tagRuleVersion, LocalDate bizDate) {
        ...
        boolean allSuppliedEvidenceInvalid = suppliedIngredientEvidence
                && validMatchedIngredientList.isEmpty();

        CtDishTagEntity entity = baseEntity(dish.getDishId(), tagHash, enumKey, modelVersion,
                promptVersion, tagRuleVersion, bizDate);
    ...
    public CtDishTagEntity stateEntity(long dishId, String tagHash, String enumKey,
                                       TagState state, String modelVersion, String promptVersion,
                                       String tagRuleVersion, LocalDate bizDate) {
        if (state != TagState.NEUTRAL && state != TagState.UNKNOWN) {
            throw new IllegalArgumentException("离线无证据标签只允许NEUTRAL或UNKNOWN");
        }
        CtDishTagEntity entity = baseEntity(dishId, tagHash, enumKey, modelVersion,
                promptVersion, tagRuleVersion, bizDate);
    ...
    private CtDishTagEntity baseEntity(long dishId, String tagHash, String enumKey,
