...
public class CtHealthReportTaskService {
	...
	/** 执行纯单向阶段迁移。 */
	public int transition(String taskId, String expectedStatus, String nextStatus, String stage, int progress) {
	...
	public int failActive(String taskId, String failCode, boolean reanalyzable) {
		return taskMapper.failActive(taskId, failCode, reanalyzable, SystemActor.HEALTH_REPORT_WORKER);
	}
	...
	/** API 侧不可逆地设置删除时间。 */
	public int markDeleted(String taskId, String userId, String companyId) {
		return taskMapper.markDeleted(taskId, userId, companyId, SystemActor.HEALTH_REPORT_API);
	}
	...
	public int deleteExpired(String taskId, LocalDateTime currentTime) {
		LambdaQueryWrapper<CtHealthReportTaskEntity> deleteWrapper = new LambdaQueryWrapper<CtHealthReportTaskEntity>();
		deleteWrapper.eq(CtHealthReportTaskEntity::getTaskId, taskId)
			.le(CtHealthReportTaskEntity::getExpireAt, currentTime)
			.and(terminalWrapper -> terminalWrapper.isNotNull(CtHealthReportTaskEntity::getDeletedAt)
				.or()
				.in(CtHealthReportTaskEntity::getStatus, TaskStatus.SUCCEEDED.name(), TaskStatus.FAILED.name()));
