...
class AnalysisTaskWorkerTest {
    ...
    void successfulFlowShouldCleanOriginalFilesAfterSuccessCommit() {
        ...
        doReturn(heartbeatFuture).when(scheduler).scheduleAtFixedRate(
        ...
        when(assembleService.assemble(any(ValidatedExtractionOutput.class), anyInt(),
                any(DegradeAccumulator.class))).thenReturn(AnalysisModules.empty());
        AnalysisExecutorProperties properties = new AnalysisExecutorProperties();
        AnalysisTaskWorker worker = new AnalysisTaskWorker(stateService, cleanupService, parseService,
                orchestrator, extractionStage, assembleService, scheduler, properties);
    ...
    void parseFailureShouldSettleBeforeAnyModelStage() {
        ...
        AnalysisExecutorProperties properties = new AnalysisExecutorProperties();
        when(stateService.claim(taskId)).thenReturn(true);
        when(parseService.parseFiles(taskId))
    ...
    void imageCapacityFailureShouldKeepDeterministicFailCode() {
        ...
        when(scheduler.scheduleAtFixedRate(any(Runnable.class), anyLong(), anyLong(),
                eq(TimeUnit.SECONDS))).thenThrow(new ImageTooLargeException(11L, 10L));
        AnalysisExecutorProperties properties = new AnalysisExecutorProperties();
        AnalysisTaskWorker worker = new AnalysisTaskWorker(stateService, cleanupService, parseService,
                orchestrator, extractionStage, assembleService, scheduler, properties);
    ...
    void r49WorkerFailureLogsShouldExcludeSensitiveExceptionMessages() {
        String taskId = "123e4567-e89b-12d3-a456-426614174000";
        List<String> prohibitedMarkerList = Arrays.asList("R49_REPORT_PAYLOAD_TOKEN",
                "R49_PERSON_NAME_TOKEN", "R49_OCR_TOKEN", "R49_HEALTH_TOKEN",
                "R49_CREDENTIAL_TOKEN", "R49_MODEL_BODY_TOKEN", "R49_ORIGIN_NAME_TOKEN");
        TaskStateService stateService = mock(TaskStateService.class);
        TaskResourceCleanupService cleanupService = mock(TaskResourceCleanupService.class);
        TaskParseService parseService = mock(TaskParseService.class);
        ParseOrchestrator orchestrator = mock(ParseOrchestrator.class);
        ...
        when(stateService.claim(taskId)).thenReturn(true);
        when(scheduler.scheduleAtFixedRate(any(Runnable.class), anyLong(), anyLong(),
                eq(TimeUnit.SECONDS))).thenThrow(new IllegalStateException(
                String.join("|", prohibitedMarkerList)));
        AnalysisTaskWorker worker = new AnalysisTaskWorker(stateService, cleanupService, parseService,
                orchestrator, extractionStage, assembleService, scheduler,
    ...
    void everyStageShouldRunInOrderAndAssembledModulesShouldReachTheResult() {
        ...
        when(extractionStage.extract(eq(parsePlan), any(DegradeAccumulator.class)))
                .thenReturn(extractionOutput);
        when(assembleService.assemble(eq(extractionOutput), eq(1), any(DegradeAccumulator.class)))
                .thenReturn(assembled);
        AnalysisTaskWorker worker = new AnalysisTaskWorker(stateService, cleanupService,
        ...
        order.verify(assembleService).assemble(eq(extractionOutput), eq(1),
    ...
    void extractionFailureShouldNotAssembleAndShouldNotWriteAnyResult() {
        ...
        verify(stateService).markFailed(taskId, FailCode.UNREADABLE);
        verify(assembleService, never()).assemble(any(), org.mockito.ArgumentMatchers.anyInt(),
                any(DegradeAccumulator.class));
