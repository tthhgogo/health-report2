...
class HealthReportControllerTest {
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(
                        new HealthReportFileController(fileUploadService, userProvider),
    ...
    void shouldExposeUploadAndAnalyzeEndpoints() throws Exception {
        ...
        when(fileUploadService.upload(any(), eq(USER_ID))).thenReturn(FILE_ID);
        when(taskCreateService.createInTransaction(anyList(), eq(USER_ID))).thenReturn(TASK_ID);

        mockMvc.perform(multipart("/api/health-report/file").file(file))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.fileId").value(FILE_ID));
        mockMvc.perform(post("/api/health-report/analyze")
    ...
    void shouldReturnConflictCodeAndBoundTaskIdWithoutSensitiveMetadata() throws Exception {
        org.mockito.Mockito.doThrow(new HealthReportException(FailCode.FILE_ALREADY_BOUND, 409, TASK_ID))
                .when(taskCreateService).precheck(anyList(), eq(USER_ID));
    ...
    void shouldMapMultipartLimitToFileTooLargeWithoutSensitiveMetadata() throws Exception {
        MockMultipartFile file = new MockMultipartFile(
                "file", "sensitive-name.pdf", "application/pdf", new byte[]{1});
        when(fileUploadService.upload(any(), eq(USER_ID)))
    ...
    /**
     * 报文畸形与字节超限必须是两个错误码：前者压缩文件重试无用，
     * 复用 FILE_TOO_LARGE 会把用户引到解决不了问题的方向上。
     */
    @Test
