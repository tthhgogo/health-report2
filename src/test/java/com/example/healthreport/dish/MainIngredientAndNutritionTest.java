...
class MainIngredientAndNutritionTest {
    ...
    void normalMainIngredientsShouldBeSelectedButTinySecondIngredientShouldNot() {
        Dish pepperPork = dish(1L, "青椒肉丝",
                ingredient("青椒", "200"), ingredient("瘦猪肉", "80"));
        Dish tomatoEgg = dish(2L, "番茄炒蛋",
                ingredient("番茄", "250"), ingredient("鸡蛋", "100"));
        Dish greensLiver = dish(3L, "青菜猪肝",
                ingredient("青菜", "180"), ingredient("猪肝", "5"));
        ...
        assertThat(matcher.match(greensLiver,
                new LinkedHashSet<String>(Collections.singletonList("猪肝"))).getState())
                .isEqualTo(TagState.NEUTRAL);
    }

    @Test
    void allUnknownWeightsShouldProduceEmptyMainSetAndNeutralNutrition() {
        Dish dish = new Dish(4L, "无重量菜", Arrays.asList(
                new DishIngredient("鲜猪肝", null), new DishIngredient("菠菜", null)));

        assertThat(resolver.resolve(dish)).isEmpty();
        assertThat(matcher.match(dish,
                new LinkedHashSet<String>(Collections.singletonList("猪肝"))).getState())
                .isEqualTo(TagState.NEUTRAL);
    }
    ...
    void aliasShouldMatchOnlyWhenItIsActuallyMainIngredient() {
        ...
        TagValue value = matcher.match(dish,
                new LinkedHashSet<String>(Collections.singletonList("猪肝")));
    ...
    private Dish dish(long id, String name, DishIngredient... ingredientArray) {
        return new Dish(id, name, Arrays.asList(ingredientArray));
    }
