...
class DishTagContractValidatorTest {
    ...
    private final java.util.List<Dish> inputDishList = Arrays.asList(
            dish(1L), dish(2L), dish(3L));
    ...
    void exactThreeWayCoverageShouldPass() {
        ...
        assertThat(validator.validate(json, "LOW_FAT", inputDishList).getHitList())
                .hasSize(1);
    }
    ...
    /**
     * 批次输入自带重复菜品 ID 是编排层的 bug，不是模型的问题。
     * 两者结果都是整批作废，但错误类型必须分开——否则排障时会去查模型和提示词，
     * 而真正的缺陷在调用方。
     */
    @Test
    ...
    private String hit(long dishId) {
        return "[{\"dishId\":" + dishId + ",\"verdict\":\"REJECT\","
                + "\"evidenceType\":\"COOKING\",\"matchedIngredients\":[],"
                + "\"reason\":\"明确做法\"}]";
    }

    private Dish dish(long dishId) {
        return new Dish(dishId, "测试菜" + dishId,
