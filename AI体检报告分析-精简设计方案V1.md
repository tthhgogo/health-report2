...
2. **三层职责边界，全文任何一节都不得越界。**
   ...
   | 层 | 负责 | 不负责 |
   |---|---|---|
   | 文件转图 | 把各种格式统一渲染成页面图 | 任何判断，也不再抽取文本 |
...
| 校验 | 规则 | 失败提示 |
|---|---|---|
| 格式 | 按 §3.1 的逐格式判定，**不信任扩展名**；**DOC/DOCX 识别即拒**（§3.2.1） | 「暂不支持该文件格式，支持PDF、JPG、PNG、OFD格式」 |
...
| DOCX | ZIP 容器 + 内含 `word/document.xml` | 不适用——**识别即拒**（§3.2.1） |
| OFD | ZIP 容器 + 内含 `OFD.xml` | ofdrw 能打开、页数 ≥ 1 |
| DOC | OLE2 复合文档头 `D0CF11E0` | 不适用——**识别即拒**（§3.2.1） |
...
——**DOC/DOCX 虽然不支持，识别逻辑仍要保留**：不区分的话 DOCX 会被当成损坏的 OFD
报「文件无法读取」，而正确提示是「暂不支持该文件格式」。
DOC 是例外，它是老的二进制 OLE2 复合文档，判 magic number 即可识别并拒绝。

#### 3.1.1 解压炸弹防御（DOCX / OFD）

攻击路径不是「用户传了个 zip 炸弹」，而是构造一个内含 `OFD.xml` 的 `.ofd`
——格式校验通过，解析时内部解压膨胀到几百 GB，一个用户就能打掉整个 Worker。
DOCX 虽然识别即拒（§3.2.1），但**识别这一步本身就要扫 ZIP 条目**，同样受下表约束。
...
| OFD | ofdrw 逐页转图 | 已实现 |
| **DOC / DOCX** | **不转图——上传识别即拒** | **第一期不支持（§3.2.1）** |

#### 3.2.1 Word 第一期不支持（2026-09-03 裁决）

旧链路用 POI 抽 Word 正文文本，不需要渲染引擎；全部转图之后这条路走不通
（纯图片契约下三次调用没有文本输入通道），而没有保真度合格的转图路线：

```
纯 Java 开源（docx4j+FOP / XDocReport）  重表格医疗文档会单元格串行 —— 数值错位是
                                        静默错读，比拒收更危险，直接排除
LibreOffice headless                    保真合格，但要新增部署依赖（LO + 中文字体 +
                                        并发/超时/沙箱管理），与 AGENTS.md §2 冲突
商业库（Aspose.Words 等）                保真合格，需采购授权
```

**裁决：第一期不支持 DOC / DOCX，上传直接返回「暂不支持该文件格式」**，
不落 file 行、不存对象。格式识别逻辑保留（§3.1：区分 DOCX 与 OFD、给出正确拒绝文案）。

这偏离需求 §3-3（DOC/DOCX 是明确要支持的格式），已登记 §12-16 待产品确认回写需求。

**恢复条件与优先路线**：先统计真实用户的 Word 上传占比；占比支撑时优先
「独立转换服务 + LibreOffice headless」（新增依赖隔离在专用容器，主应用零改动），
其次评估商业库采购。恢复时需一并恢复等效页折算、Worker 二次容量裁决与相关测试
（开发方案 §5.4 记录了被移除的机制清单）。
...
第一期不支持 Word（§3.2.1）后，所有支持格式的页数在上传时即可精确计算：
PDF / OFD 为真实页数，图片恒为 1。**不存在等效页折算，也没有 Worker 二次业务容量裁决。**
...
模块一至三保持 Schema 数组顺序并用 `page` 校验跨文件位置；
...
```
分组顺序   sections 数组顺序（模型按报告阅读顺序给出），多文件时先按 page 收敛
...
不满足时稳定重排（不丢弃条目）。多文件时先按 `page` 收敛，再按数组顺序。
...
过敏提醒区按 `reject` 数组顺序展示，多文件时先按 `page` 收敛。
...
> **这不满足需求 §7-3。** 需求要求每个过敏原都列避免食材和易忽略食物、
...
**页面形态：「适宜多吃」与「忌吃少吃」两个清单，内容是食材，不是建议卡片。**

> **这是产品确认的形态，与需求 §7-3 / §7-5 的字面有三处偏离，全部已确认接受：**
>
> | 偏离 | 需求原文 | 现状 |
> |---|---|---|
> | 不按三维度分区 | §7-3「按三个维度分区展示，每个维度独立成卡片区」 | 按方向分成两个食材清单 |
> | 不标来源 | §7-3「每条建议旁标注来源，引用报告原文」 | 食材条目旁不挂来源 |
> | 食材层面合并 | §7-5「各自独立展示，不做交叉关联或合并」 | 三个维度的食材合并成两个清单 |
>
> **需求文档需同步修订这三条。** `section` / `itemNo` / `quote` 三个字段仍在契约里
> （§4.2.2）——它们是排障与后续恢复来源标注的唯一依据，**不要因为当前不展示就删掉**。

#### 7.6.1 两个清单怎么算出来

```
nutritionSet     = ∪ { NutritionContent[enumKey].recommendIngredients
                       | recommend 里 dimension = NUTRITION }
dietRecommendSet = ∪ { DietRequirementContent[enumKey].recommendIngredients
                       | recommend 里 dimension = DIET }

allergenSet      = ∪ { AllergenGroups[enumKey].avoidIngredients ∪ .hiddenFoods
                       | reject 里 dimension = ALLERGEN }
dietAvoidSet     = ∪ { DietRequirementContent[enumKey].avoidIngredients
                       | reject 里 dimension = DIET }

忌吃少吃 = allergenSet ∪ dietAvoidSet
适宜多吃 = (nutritionSet ∪ dietRecommendSet) − 忌吃少吃
```

**两侧都先取并集，最后做一次差集。** 需求 §8-3 对菜品定的是「规则冲突时以不推荐优先」，
食材清单沿用同一条——一个食材只要出现在忌吃少吃侧，无论它在推荐侧命中了几条，一律移除。

**差集必须放在最后，不能在各维度内部分别做。** 报告同时说「补铁」和「低脂饮食」时，
猪肝在 `nutritionSet` 里、也在 `dietAvoidSet` 里；只在营养补充维度内部判断的话，
另一个维度的禁忌就漏掉了。**先各自取并，再统一减一次**，是唯一不会漏的顺序。

#### 7.6.2 差集用宽松匹配，方向偏向多减

```
适宜多吃里的某个食材 F，只要【包含】忌吃少吃侧任意一个 matchWord，就从适宜多吃中移除
```

```
过敏原「虾蟹类」的 matchWord 含「虾」
适宜多吃候选里有「虾仁」「基围虾」
  → 两者都含「虾」→ 全部移除          ✓ 正确
```

**按 displayName 精确相等做差集是不够的**：过敏原常量表里的 `matchWord` 本来就比
`displayName` 宽（§7.5），而差集这一步的误判代价是不对称的——
少推荐一个食材只是少一条信息，把过敏原推给用户是一级红线（§0-6）。**宁可多减。**

`hiddenFoods`（虾丸、蟹棒、XO酱）**必须一并进入忌吃少吃**：需求 §7-3 明确要求展示
「易忽略的含该过敏原的常见食物」，它们在两个清单形态下没有别的地方可去。

#### 7.6.3 `OTHER` 与被安全闸抑制的条目不进清单

`enumKey = OTHER` 没有食材表，被 §7.3 安全闸抑制的条目按 `OTHER` 路径处理
——两者都**不产生任何食材**，因此在本形态下不可见。

> **这是本形态最大的一处信息损失，产品已确认接受。**
> 低蛋白、限钾、限碘这类**故意不做归一化**的方向性禁忌全部落在 `OTHER` 里
> ——报告写了「建议优质低蛋白饮食」，页面上什么都不会出现。
> 这些条目仍在结果对象里下发（带 `quote` 与 `section`），前端当前不渲染；
> **`OTHER` 占比进入离线评测报告**，异常时应重新评估本形态；生产请求不新增健康数据计数器。
...
> **文案与需求 §7-4 不同**：那里是按三维度写的（「本次体检报告未涉及过敏原相关内容」等三句），
> 而本形态没有三个分区。**上表文案待产品最终确认**，需求文档需同步修订。

**全模块不出现任何提示、说明或警示文字**，只有两个食材清单、上表的空态句
以及 §7.8 的底部声明（产品决策）。

**「适宜多吃」被差集减空是一种正常结果**，与「本来就没有推荐」走同一句文案
——报告说了补铁、但推荐食材全部撞上过敏原时，页面显示的是空态，不解释原因。

> **已知接受的风险（本形态下更严重）：**
>
> ① 报告压根没做过敏原筛查时，页面与「做了筛查但全阴性」完全一样，都显示空态。
>    系统不会告知用户「推荐结果未考虑过敏因素」，而本版又没有用户自填过敏原的入口。
>
> ② **`OTHER` 与被安全闸抑制的条目在本形态下完全不可见**（§7.6.3）。
>    报告写了「建议优质低蛋白饮食」，两个清单都不会出现任何东西，页面看起来像
>    「报告没提饮食」——而它其实提了一条方向性禁忌。
>
> 两条兜底都只剩 §7.8 与 §8.11 的模块声明。此为产品明示决策。
...
2. 按 fileIndex 顺序逐文件转图，拼成一个全局图序列（§3）  ← 新
     PDF / OFD / 图片各自的转图路径（DOC/DOCX 第一期不支持，上传即拒，§3.2.1）
...
39. **【已移除】**原「Word 上传下界预筛 + Worker 精确容量裁决」随第一期不支持 Word 删除；
    页数裁决全部在创建时同步完成（§3.2.1、§3.4.1）
...
| 8 | ~~Word 等效页折算系数~~ | **【搁置】**第一期不支持 Word（§3.2.1），恢复支持时重启验证 | — |
| 9 | **Word 上传占比与内嵌图片形态占比** | 灰度期统计被 `UNSUPPORTED_FORMAT` 拒掉的 DOC/DOCX 占比 | 这是 §3.2.1「恢复条件」的判据：占比高则重启 Word 支持评估（独立转换服务 + LibreOffice 优先） |
...
| 16 | **DOC / DOCX 第一期不支持（2026-09-03 裁决，§3.2.1）**——无保真合格的纯 Java 转图路线，LibreOffice/商业库需新增依赖或采购，Word 上传占比未证实前不值得付出。偏离需求 §3-3，需产品确认并回写需求；上传文案同步改为「暂不支持该文件格式，支持PDF、JPG、PNG、OFD格式」 |
