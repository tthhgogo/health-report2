...
| 校验 | 规则 | 失败提示 |
|---|---|---|
| 格式 | 按 §3.1 的逐格式判定，**不信任扩展名**；**旧版 DOC 识别即拒**（§3.2.1，DOCX 于 2026-09-05 恢复支持） | 「暂不支持该文件格式，支持PDF、JPG、PNG、OFD、DOCX格式」 |
...
| **DOC** | **不转图——上传识别即拒** | **不支持（§3.2.1）** |

#### 3.2.1 Word 支持口径（2026-09-05 改裁：DOCX 恢复，DOC 仍不支持）
...
（表格边框、异常值高亮、文字形式的 ↑ 提示箭头），评估过程与产物见
`DocxToPdfConverterTest` 及实验测试。图片不属于现行保真验收范围，据此改裁：

**DOCX 恢复支持**：docx4j 排版转 PDF（XSL-FO/FOP，纯 Java、无 LibreOffice/商业库依赖）
后完全复用 PDF 渲染路径。

**DOCX 图片一律丢弃（2026-09-05 确认）**：本工程对 DOCX 只关注文字与表格，
不关注其中的图片。无论内嵌还是外链，无论位于正文、表格、页眉或页脚，图片均不进入
转换产物，也不参与后续模型分析；医学影像、带文字的截图与扫描图片同样处理，
...
该规则仅针对 **DOCX 内的图片对象**，不改变独立 JPG/PNG、扫描版 PDF 或 OFD 的既有转图链路，
也不取消发给 LLM-A 的页面图。
...
字体环境   思源黑体（SourceHanSansCN-Regular.otf，SIL OFL 1.1，许可证随包分发）
          ...
          替换到该字体；内置加载失败退回系统字体候选，两者都不可用才按
          SERVER_ERROR 失败——环境问题不得归因为用户文件不可读，
          更不能把中文渲染成 # 后静默送给模型。
页数口径   DOCX 没有固有分页，其精确页数 = 按上述规则丢弃图片后确定性排版的 PDF 页数；
          不承诺与原 Word 文档含图片时的页数相同。上传预检、Worker 复核与渲染须使用
          同一图片丢弃规则和字体环境，页数保持一致（§3.4.1 契约保持）。
```

**DOC 仍不支持**：老的二进制 OLE2 格式，识别即拒、返回「暂不支持该文件格式」。
需求 §3-3 的偏离范围相应收窄为仅 DOC，登记 §12-16。

已知成本：一次 DOCX 任务共发生三次排版转换（上传预检、Worker 读回复核、Worker 渲染），
...
| 16 | **Word 支持口径（2026-09-05 改裁，§3.2.1）**——DOCX 恢复支持：原裁决前提「无保真合格的纯 Java 转图路线」被真实样本评估证伪，docx4j 转图路线落地，思源黑体（SIL OFL）随应用内置、部署零字体依赖。**DOC 仍不支持**（OLE2 老格式），偏离需求 §3-3 的范围收窄为仅 DOC，需产品确认并回写需求；上传文案同步改为「暂不支持该文件格式，支持PDF、JPG、PNG、OFD、DOCX格式」 |
